  ...
  getCities = () => {
    ...
    getPin = (station) => {
        if (this.state.needABike) {
            return (
                station.free_bikes === 0 ? { uri: 'ic_place_grey_24px' } : (
                    station.free_bikes < station.extra.slots * 0.33 ? { uri: 'ic_place_red_24px' } : (
                        station.free_bikes < station.extra.slots * 0.50 ? { uri: 'ic_place_amber_24px' } : { uri: 'ic_place_green_24px' }
                    )
                )
            );
        } else {
          return (
              station.empty_slots === 0 ? { uri: 'ic_place_grey_24px' } : (
                  station.empty_slots < station.extra.slots * 0.33 ? { uri: 'ic_place_red_24px' } : (
                      station.empty_slots < station.extra.slots * 0.50 ? { uri: 'ic_place_amber_24px' } : { uri: 'ic_place_green_24px' }
                  )
              )
          );
        }
              ...
              onRegionChangeComplete={this.onRegionChange}>
              {
                  this.state.stations.map((station) => {
                      return (<MapView.Marker
                          ...
                          coordinate={ { latitude: station.latitude, longitude: station.longitude } }></MapView.Marker>);
                  })
              }
          </MapView>
          <View style={ styles.controls }>
            <View style={ styles.button }>
                <RoundIconButton
                    key="biker"
                    color="#607D8B"
                    icon={ this.state.needABike ? 'ic_directions_bike_white_24px' : 'ic_local_parking_white_24px' }
